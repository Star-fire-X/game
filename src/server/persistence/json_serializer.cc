...
SerializationResult JsonSerializer::SerializeImpl(const void* component, const char* type_name) {
    try {
        std::type_index idx(std::type_index(component ? typeid(*component) : typeid(void)));
        auto serializer = GetSerializer(idx);
...
bool JsonSerializer::DeserializeImpl(const std::vector<uint8_t>& data, void* component,
    ...
    try {
        ...
        std::type_index idx(std::type_index(component ? typeid(*component) : typeid(void)));
        auto deserializer = GetDeserializer(idx);
...
JsonSerializer::SerializeFn JsonSerializer::GetSerializer(const std::type_index& idx) {
    auto it = serializers_.find(idx);
...
JsonSerializer::DeserializeFn JsonSerializer::GetDeserializer(const std::type_index& idx) {
    auto it = deserializers_.find(idx);
