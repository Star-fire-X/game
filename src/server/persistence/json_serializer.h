...
class JsonSerializer : public ISerializer {
 public:
    ...
    void RegisterCustom(
        ...
        serializers_[idx] = [serialize](const void* ptr) {
            return serialize(*static_cast<const T*>(ptr));
        };

        deserializers_[idx] = [deserialize](const json& j, void* ptr) {
 ...
 private:
    std::map<std::type_index, SerializeFn> serializers_;
    std::map<std::type_index, DeserializeFn> deserializers_;

    /**
     * @brief Get serializer for a type
     */
    SerializeFn GetSerializer(const std::type_index& idx);

    /**
     * @brief Get deserializer for a type
     */
    DeserializeFn GetDeserializer(const std::type_index& idx);
};
