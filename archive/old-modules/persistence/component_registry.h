...
class ComponentRegistry {
 public:
    ...
    bool RegisterComponent(
        ...
        std::function<bool(const std::vector<uint8_t>&, T&)> deserialize,
        std::function<bool(const T&)> validate = nullptr) {
        SerializeCallback serialize_cb = [serialize](const void* ptr) {
            return serialize(*static_cast<const T*>(ptr));
        };
        DeserializeCallback deserialize_cb =
            [deserialize](const std::vector<uint8_t>& data, void* ptr) {
                return deserialize(data, *static_cast<T*>(ptr));
            };
        ValidateCallback validate_cb = nullptr;
        if (validate) {
            validate_cb = [validate](const void* ptr) {
                return validate(*static_cast<const T*>(ptr));
            };
        }
        return RegisterComponent<T>(
            name, std::move(serialize_cb), std::move(deserialize_cb), std::move(validate_cb));
    }
    ...
    bool RegisterComponent(
        ...
        std::function<bool(const T&)> validate = nullptr) {
        auto deserialize_cb = [deserialize](const std::vector<uint8_t>& data, T& out) {
            out = deserialize(data);
            return true;
        };
        return RegisterComponent<T>(
            name, std::move(serialize), std::move(deserialize_cb), std::move(validate));
    }

    /**
     * @brief Register a component type serializer (type-erased)
     */
    template<typename T>
    bool RegisterComponent(
        const std::string& name,
